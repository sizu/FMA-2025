...
import { ArrowLeft, TrendingUp, Check, X, Trash2, Clock } from 'lucide-react';
...
const InvestmentForm: React.FC<InvestmentFormProps> = ({ userRole, onBack }) => {
  const { investments, addInvestment, updateInvestmentStatus, deleteInvestment } = useAppContext();
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const numericValue = ['perUnitPrice', 'amountOfProducts', 'totalCost'].includes(name) ? 
    ...
    setFormData(prev => {
      ...
      // Auto-calculate total cost when per unit price or amount changes
      if (name === 'perUnitPrice' || name === 'amountOfProducts') {
        updated.totalCost = updated.perUnitPrice * updated.amountOfProducts;
      }
  ...
  const totalInvestment = investments.filter(i => i.status === 'approved').reduce((sum, i) => sum + i.totalCost, 0);
    ...
    <div className="space-y-6 fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <h2 className="text-3xl font-bold text-gray-800">Investment Tracking</h2>
        </div>
        <button
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-2xl shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-bold">{totalInvestment.toLocaleString()} Tk</p>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h3 className="text-2xl font-bold text-gray-800">Add Investment</h3>
                <button
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-xl font-semibold text-gray-800">Investment Records</h3>
        </div>
        <div className="overflow-x-auto">
              ...
              {investments.map((investment) => (
                <tr key={investment.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {investment.invoiceNo}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {investment.productName}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {new Date(investment.buyingDate).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {investment.perUnitPrice} Tk
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {investment.amountOfProducts} {investment.unit}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                    {investment.totalCost.toLocaleString()} Tk
                  </td>
          ...
          {investments.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">Get started by adding an investment record.</p>
            </div>
          )}
        </div>
      </div>
